...
menuPageSelectControls.forEach((control) => {
	...
	control.addEventListener('click', (e) => {
		...
		// @ts-ignore
		const currentPageOrder = getEls('.control--panel__page')[0].getAttribute('data-page-order');
		const newPage = getEls(`[data-page="${selectsPage}"]`);
		// @ts-ignore
		...
		if ( el.classList.contains( 'is-active') ) {
			...
			// @ts-ignore
			currentPage.classList.remove( ...pageAnimClassList ).add( outroClass );
			// @ts-ignore
			menuPageSelectControls.classList.remove( 'is-active' );
			// @ts-ignore
